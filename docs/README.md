...
### 크리스마스 할인 적용 서비스
본 프로그램은 사용자로부터 구매 내역을 입력 받고 날짜, 메뉴 구성에 따라 할인 내역을 충첩해서 알려주는 서비스이다. <br><br>

### 도메인의 역할

#### VisitDate <br>
* 구매 날짜를 검증하고 관리한다. <br><br>

#### OrderMenu <br>
* 구매 메뉴와 수량을 검증한다. 그리고 해당 메뉴의 총 구매 금액을 계산하는 역할을 수행한다. <br><br>

#### Order <br>
* `OrderMenu`와 `VisitDate`를 가지며 주문과 관련된 추가적인 검증을 수행한다. <br>
* ex) 총 구매 개수 검증, 음료만 구매했는지, 중복 메뉴가 있는지 <br>
* `Order` 와 `OrderMenu` 는 1:N 관계를 가지며 전체 구매 금액을 계산하는 역할을 수행한다. <br><br>

#### DiscountManager <br>
* 할인과 관련된 관리를 책임진다. DDay, 평일, 주말, 특별, 증정품에 관련된 할인 내역을 계산하여 알려준다. <br><br>

#### Bill <br>
* 모든 구매 내역, 할인 내역을 가지고 있으며 배지를 생성하는 역할을 가진다.
<br><br>
